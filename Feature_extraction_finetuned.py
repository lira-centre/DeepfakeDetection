...
model_VT = ViTForImageClassification.from_pretrained('weights\pytorch_model_vit_oxfordiitpets_finetune.bin',config='weights\config_vit_oxfordiitpets_finetune.json', local_files_only=True)
feature_extractor_VT = nn.Sequential(*list(model_VT.children())[:-1])
...
def extractor(img_path):
    ...
    try:
        ...
        # For fine-tune
        y = feature_extractor_VT(x)['last_hidden_state'][:, 0]
